...
adminRouter.get('/api/estadisticas', async (req, res) => {
  ...
  // Consulta el libro para el día elegido (desde) o el de hoy; si hay rango
  // (desde y hasta), también trae las noches reservadas de ese período.
  const libro = await ocupacionDelLibro(desde || null, desde || null, hasta || null);
  if (libro) {
    ...
    habitaciones.nochesReservadasRango = libro.nochesReservadasRango;
...
adminRouter.get('/api/vik/probar', async (req, res) => {
  const r = await probarPlantilla(String(req.query.telefono || ''));
  res.status(r.ok ? 200 : 400).json(r);
});
